...
Ändere die Standard-Farbpaletten, um Zugänglichkeit und Ästhetik zu verbessern. Wir vergleichen zwei Varianten und schauen, welche besser zugänglich ist (Stichwort: Farbsehschwäche).

Variante 1: `scale_color_colorblind()`
...
Variante 2: `scale_color_manual()` mit eigenen Farben. Wir nutzen hier nur die Pinguine der Insel Biscoe, indem wir die Daten mit `filter()` eingrenzen.

```{r}
penguins_biscoe <- penguins |>
  filter(island == "Biscoe")

ggplot(data = penguins_biscoe,
       mapping = aes(x = flipper_length_mm,
                     y = body_mass_g,
                     color = species,
                     shape = species)) +
...
Welche der beiden Varianten ist für Personen mit Farbsehschwäche besser geeignet? Warum?
...
- CH-Tastatur Mac: Alt/Option + N
- CH-Tastatur Windows: AltGr + ^, danach Leertaste
